...
def extract(tag):
    ...
    for i in range(13):
        np.save("data/train_data_"+ str(i) + "_"+ tag, X_train[:,i,:])
        np.save("data/test_data_"+ str(i) + "_"+ tag, X_test[:,i,:])

        np.save("data/dev_data_"+ str(i) + "_"+ tag, X_dev[:,i,:])

    np.save("data/train_label_"+ tag, y_train)

    np.save("data/test_label_"+  tag, y_test)

    np.save("data/dev_label_"+  tag, y_dev)
...
sum1 = []
for i in range(len(tokens['target'])):
    for j in range(len(tokens['target'][i])):
        sum1.append(tokens['target'][i][j])
    sum1 = list(set(sum1))
print(sum1)
sum_dict = dict()
for t in sum1:
