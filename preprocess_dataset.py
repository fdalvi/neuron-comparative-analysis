...
parser.add_argument('--words', type=str, default='word.txt',
                       help='Input word file')
parser.add_argument('--labels', type=str, default='label.txt',
...
transformers_extractor.extract_representations(args.model_name_or_path,
...
def extract(tag):
    ...
    for i in range(13):
        np.save(os.path.join(args.out_path ,"train_data_"+ str(i) + "_"+ tag), X_train[:,i,:])
        np.save(os.path.join(args.out_path ,"test_data_"+ str(i) + "_"+ tag), X_test[:,i,:])
        np.save(os.path.join(args.out_path ,"dev_data_"+ str(i) + "_"+ tag), X_dev[:,i,:])
...
activations, num_layers = data_loader.load_activations(os.path.join(args.input_folder, "activations.json"), 768)
tokens = data_loader.load_data(os.path.join(args.input_folder, args.words), os.path.join(args.input_folder, args.labels), activations, 512 )
