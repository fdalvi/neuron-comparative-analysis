...
def compute(tag, layer, number, lamda, method1_name, method2_name):
    ...
    method1 = np.loadtxt("neurons_splits/" + method1_name + "/"+tag+"/"+str(layer)+"_neurons.txt",dtype = np.int32)
    method2 = np.loadtxt("neurons_splits/" + method2_name + "/"+tag+"/"+str(layer)+"_neurons.txt",dtype = np.int32)

    method1_values = np.loadtxt("neurons_splits/" + method1_name + "/"+tag+"/"+str(layer)+"_values.txt",dtype = np.float)
    method2_values = np.loadtxt("neurons_splits/" + method2_name + "/"+tag+"/"+str(layer)+"_values.txt",dtype = np.float)
    ...
    result = []
    values_all = method1_values + lamda * method2_values
    ranking = np.argsort(values_all)[::-1]
    for n in number:
        ...
        rank = ranking[:n]
        score, score_dev = train_dev_and_eval(X_train, y_train,X_dev, y_dev, X_test, y_test, rank)
...
for la in lamda:
    for t in tags:
        ...
        for i in range(13):
            ...
            np.savetxt("result_combine/" + t + "_" + str(i) + "_combine" + str(la) + "_lasso_ridge" + ".txt", compute(t,i,l, la, "lca_ridge_01", "lca_lasso_001"))
